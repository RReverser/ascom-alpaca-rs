use crate::rpc::OpaqueResponse;
use crate::transaction::OpaqueParams;
...
pub(crate) struct ServerRequestTransaction {
...
pub(crate) fn server_handler<
    Resp: Response,
    RespFut: Future<Output = axum::response::Result<Resp>>,
>(
    path: impl Display,
    is_mut: bool,
    mut raw_opaque_params: OpaqueParams,
    make_response: impl FnOnce(&mut OpaqueParams) -> RespFut,
    ...
    async move {
        let response = make_response(&mut raw_opaque_params).await?;
        if !raw_opaque_params.0.is_empty() {
            tracing::warn!("Unknown parameters: {:?}", raw_opaque_params.0.keys());
        }
...
impl Client {
    ...
    pub(crate) async fn request<Resp: Response>(
        ...
        fill: impl FnOnce(reqwest::RequestBuilder) -> reqwest::RequestBuilder,
    ) -> anyhow::Result<Resp> {
        ...
        .instrument(span)
        .await
        .with_context(|| format!("Failed to send Alpaca request to {path}"))
    }
}
